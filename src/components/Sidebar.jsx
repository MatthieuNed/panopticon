      ...
      <Paper elevation={10} className={classes.paper}>
        <form className={classes.root} noValidate autoComplete="off">
          <Grid container className={classes.gridContainer}>
            ...
            <Grid item xs={12} md={6} className={classes.padding}>
              <Typography gutterBottom variant="h6">Appeler un ami</Typography>
              <TextField label="ID à appeler" value={idToCall} onChange={(e) => setIdToCall(e.target.value)} fullWidth />
              {callAccepted && !callEnded ? (
                <Button variant="contained" color="danger" fullWidth onClick={leaveCall} className={classes.margin}>
                  arrêter la video
                </Button>
              ) : (
